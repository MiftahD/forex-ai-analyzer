...
div[class*="st-key-sq_"] button:hover {
...
def build_chart(
    ...
    # Build per-row specs — only row 1 (price chart) needs a secondary y-axis for volume
    ...
    if "Volume" in df.columns and df["Volume"].sum() > 0:
        ...
        # Cap secondary y-axis so volume bars only occupy the bottom ~25% of the price panel
...
def _to_gemini_contents(messages: List[Dict]) -> List["genai_types.Content"]:
    """
    Convert the FULL Streamlit session message history into Gemini
    Content objects to be sent as `contents` on every request.
...
def stream_gemini(
    ...
    Uses the stateless generate_content_stream API (full history sent as
    `contents` each call) — avoids the 'client has been closed' issue that
    can occur with the chats.create() session API across Streamlit reruns.
    """
...
def call_gemini(
    ...
    Uses the stateless generate_content API (full history sent as
    `contents` each call).
    """
    try:
        client   = genai.Client(api_key=api_key)
